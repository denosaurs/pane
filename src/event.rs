...
impl From<WindowEvent<'_>> for SurfaceWindowEvent {
  fn from(window_event: WindowEvent) -> Self {
    match window_event {
      ...
      WindowEvent::HoveredFileCancelled => SurfaceWindowEvent::HoveredFileCancelled,
      WindowEvent::ReceivedCharacter(ch) => SurfaceWindowEvent::ReceivedCharacter(ch),
      ...
      WindowEvent::ModifiersChanged(modifiers) => SurfaceWindowEvent::ModifiersChanged(modifiers),
      ...
      WindowEvent::CursorEntered { device_id } => SurfaceWindowEvent::CursorEntered {
        device_id: hash(device_id),
      },
      ...
      WindowEvent::Touch(touch) => SurfaceWindowEvent::Touch(Touch::from(touch)),
      ...
      WindowEvent::ThemeChanged(theme) => SurfaceWindowEvent::ThemeChanged(Theme::from(theme)),
    }
  }
}
...
impl From<DeviceEvent> for SurfaceDeviceEvent {
  fn from(device_event: DeviceEvent) -> Self {
    match device_event {
      ...
      DeviceEvent::MouseMotion { delta } => SurfaceDeviceEvent::MouseMotion { delta },
      DeviceEvent::MouseWheel { delta } => SurfaceDeviceEvent::MouseWheel { delta },
      DeviceEvent::Motion { axis, value } => SurfaceDeviceEvent::Motion { axis, value },
      DeviceEvent::Button { button, state } => SurfaceDeviceEvent::Button { button, state },
      DeviceEvent::Key(keyboard_input) => SurfaceDeviceEvent::Key(keyboard_input),
